...
(comment
  ...
  (let [db  (-> (datascript/empty-db {:aka { :db/cardinality :db.cardinality/many }})
                (datascript/with [[:db/add 1 :name "Ivan"]])
                (datascript/with [[:db/add 1 :name "Petr"]])
                (datascript/with [[:db/add 1 :aka  "Devil"]])
                (datascript/with [[:db/add 1 :aka  "Tupen"]]))]
